...
public class QuestComp_MedicalEmergency : WorldObjectComp, IThingHolder
{
    ...
    public override void CompTick()
    {
        ...
        if (injured.NullOrEmpty())
        {
            injured = (from p in mapParent.Map.mapPawns.AllPawns
                ...
                select p).ToList();
            RaD_Mod.LogMessage($"Found {injured.Count} injured pawns from the {DownedFaction.Name}");
        }
        else
        {
            // RaD_Mod.LogMessage($"Active with {injured.Count} in list and max of {maxPawns}.");
            foreach (var pawn in injured)
            {
    ...
    private void GenerateNewFaction()
    {
        ...
        RaD_Mod.LogMessage($"Created faction {faction.Name}");
